...
> 一款由 AI 驱动的开源 macOS 菜单栏截图翻译工具，帮助你快速识别并翻译屏幕文本。使用翻译功能前，用户需要自行申请并配置受支持 AI 服务供应商的 API Key（AK）。

OpenScreenTranslate 基于 Tauri 2、Rust、ScreenCaptureKit 与 Apple Vision 构建。
应用常驻菜单栏，不显示 Dock 图标，并支持截图翻译与手动输入翻译。OCR 在本机完成，
待翻译文本会发送至用户选择的 AI 服务。
